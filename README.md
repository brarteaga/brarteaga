# 👋 Hola, soy Begoña Rodríguez Arteaga

🔐 Cybersecurity | Governance, Risk & Compliance (GRC) | Cybersecurity Compliance

![Cybersecurity](https://img.shields.io/badge/Cybersecurity-Professional-blue)
![GRC](https://img.shields.io/badge/GRC-Governance%20Risk%20Compliance-green)
![ISO27001](https://img.shields.io/badge/ISO-27001-orange)
![ENS](https://img.shields.io/badge/ENS-Compliance-red)
![Security+](https://img.shields.io/badge/CompTIA-Security%2B-lightgrey)

---

## 👩‍💻 About me

¿Qué hace una periodista y politóloga analizando vulnerabilidades y normativas?

Si me lo hubieran preguntado hace unos años, parecería el argumento de una serie de Netflix. Pero aquí estamos… uniendo los puntos.

Tras **7 años trabajando en Comunicación e Investigación Social**, analizando cómo piensa la gente y cómo toma decisiones, descubrí algo importante: si descuidas el **factor humano**, la seguridad es papel mojado.

Por eso decidí dar el salto al **Bootcamp de Ciberseguridad de The Bridge**, para entender cómo funcionan realmente los sistemas y aprender a **romper las cosas desde dentro para poder protegerlas mejor**.

Mi superpoder es actuar como **traductora entre mundos**:

🤖 Tecnología  
⚖️ Regulación  
🏢 Negocio  

O dicho de otra forma: hablo **humano, leyes y código**.  
Soy una especie de *Duolingo universal*, capaz de explicar el RGPD a un developer o traducir un pentest al lenguaje de negocio que necesitan los stakeholders.

---

## 🎓 Formación actual

Actualmente estoy en lo que llamo mi **triatlón del cumplimiento**:

🎓 **Máster Experto en Derecho Digital** — Universidad de Deusto  
Para que las leyes tecnológicas no tengan secretos.

🛡 **Postgrado en GRC Corporativo** — Mondragon Unibertsitatea  
Trabajando con marcos como **ISO 27001, ENS, NIS2, CRA y DORA**.

💻 **CompTIA Security+**  
Para seguir hablando con soltura con los sistemas.

---

## 🎯 Áreas de interés

- Cybersecurity Compliance  
- Regulatory Compliance  
- Governance, Risk & Compliance (GRC)  
- Gestión de riesgos en ciberseguridad (ISO 27005)  
- Auditoría de seguridad (ISO 27001 / ENS)  
- Cumplimiento normativo y protección de datos  
- Vulnerability assessment  
- Malware analysis  
- Ética e Inteligencia Artificial  

---

## 🚀 Proyectos

- 🔐 Cybersecurity Portfolio  
- 🔎 Pentesting Metasploitable3  
- 🦠 Malware Analysis (Petya)  
- 📊 GRC Risk & Compliance Framework  

---

## 🌍 Intereses

Me interesa especialmente la intersección entre:

- 🤖 Ética e Inteligencia Artificial  
- 🌐 Ciberseguridad y Relaciones Internacionales
- 🧠 Cultura y concienciación en Ciberseguridad
- 🏢 Cultura organizacional de seguridad

Mi objetivo es contribuir a que la ciberseguridad deje de ser **“lo que hay que hacer para cubrir el expediente”** y pase a formar parte de la **cultura corporativa, económica y social**.

---

## ☕ Let's talk

Si buscas a alguien que sabe lo que es una **inyección SQL** y que además puede redactar una **política de seguridad sin que nadie se duerma…**... aquí tienes cómo contactar conmigo.

💼 LinkedIn  
https://www.linkedin.com/in/brarteaga  

📧 Email  
begonarodriguezarteaga@gmail.com
